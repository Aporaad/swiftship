...
async function startServer() {
  ...
  // Cloud Run does not preserve in-process timers. The managed Heartbeat job calls this route every 6 hours.
  app.post('/api/scheduled/sync-active-orders', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({
        error: error?.message ?? 'Tracking synchronization failed',
