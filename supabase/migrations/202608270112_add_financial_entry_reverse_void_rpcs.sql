-- عمليات حساسة للقيود الجديدة فقط: حذف المسودة، عكس القيد المرحّل، وإبطال المسودة.
-- لا تغير RLS ولا تحذف أي سجل من الجداول المالية القديمة.

BEGIN;

CREATE OR REPLACE FUNCTION public.delete_financial_entry_draft(p_entry_id text)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  IF entry_record.posting_status <> 'draft' THEN RAISE EXCEPTION 'يمكن حذف المسودات فقط؛ القيود المرحلة تعالج بالعكس أو الإبطال.'; END IF;
...
BEGIN
  ...
  IF entry_record.reversal_entry_id IS NOT NULL OR EXISTS (SELECT 1 FROM public.main_entry WHERE reverses_entry_id = p_entry_id) THEN
  ...
  UPDATE public.main_entry SET reversal_entry_id = result->>'id' WHERE id = p_entry_id;
...
BEGIN
  ...
  SET posting_status = 'voided', updated_by_uid = p_voided_by_uid
  WHERE id = p_entry_id;
