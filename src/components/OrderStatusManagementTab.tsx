...
export default function OrderStatusManagementTab({ isAr }: OrderStatusManagementTabProps) {
  ...
  const handleSaveEntry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: AutoEntryRule = {
        ...
        amountSource: entryFormData.amountSource || 'order_total',
  ...
  const AMOUNT_SOURCE_OPTIONS = [
    ...
    { id: 'amount_remaining', labelAr: 'المبلغ المتبقي على العميل', labelEn: 'Remaining Balance' },
    { id: 'sourcing_cost', labelAr: 'تكلفة المنتجات/السورسينج', labelEn: 'Sourcing Cost' },
    { id: 'shipping_cost', labelAr: 'تكلفة الشحن الدولي/المحلي', labelEn: 'Shipping Cost' },
    { id: 'delivery_wage', labelAr: 'أجور توصيل المندوب', labelEn: 'Courier Delivery Wage' },
    { id: 'courier_commission', labelAr: 'عمولة المندوب/الوكيل', labelEn: 'Courier Commission' },
    { id: 'company_profit', labelAr: 'صافي أرباح الشركة', labelEn: 'Company Profit' }
  ];
                  ...
                  >
                    ...
                    <div className="pt-4 border-t border-slate-900 mt-4 flex items-center justify-between">
                      <span className="text-[10px] text-slate-500 font-mono">
                        {isAr ? 'مصدر المبلغ: ' : 'Source: '}
                        {AMOUNT_SOURCE_OPTIONS.find(o => o.id === entry.amountSource)?.labelAr || entry.amountSource || 'إجمالي الطلب'}
                      </span>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 text-start">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto space-y-4 flex-1">
              ...
              <div className="space-y-1">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider">
                  {isAr ? 'مصدر أو صيغة مبلغ القيد التلقائي' : 'Amount Source/Formula'}
                </label>
                <select
                  value={entryFormData.amountSource || 'order_total'}
                  onChange={(e) => setEntryFormData(prev => ({ ...prev, amountSource: e.target.value as any }))}
                  className="w-full bg-black/40 border border-slate-850 text-white rounded-xl p-3 outline-none focus:border-[#d4af37] text-xs font-bold cursor-pointer"
                >
