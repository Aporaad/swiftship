  ...
  const getUnifiedLedger = () => {
    ...
    if (entityType === 'customer') {
      ...
      orders.forEach(order => {
        const totalCost = parseFloat(order.totalCostYER || order.totalPrice || 0);
        ...
        if (amtPaid > 0) {
          ledger.push({
            ...
            amount: amtPaid,
            module: 'payment',
            title: isAr ? 'مقبوضات شحن مسددة' : 'COD Payment Settled',
            description: isAr 
              ? `كاش مسدد للشحنة رقم: ${order.orderNumber || 'ALX-CR'}` 
              : `Cash paid for shipment #${order.orderNumber || 'ALX-CR'}`,
            ref: order.orderNumber || order.id
          });
        }
      });
      ...
      orders.forEach(order => {
        ...
        if (status !== 'تم التسليم' && status !== 'Delivered') return;
        ...
        ledger.push({
          ...
          amount: collectedCOD,
          module: 'order',
          title: isAr ? 'مقبوضات شحنة لم يتم توريدها' : 'Delivered Shipment COD Collected',
          description: isAr 
            ? `تم توصيل الطلب رقم: ${order.orderNumber || 'ALX-CR'} وتحصيل غراس الكاش بعهدة المندوب` 
            : `Delivered shipment #${order.orderNumber || 'ALX-CR'} - Cash collected at door`,
          ref: order.orderNumber || order.id
        });
      });
