...
export default function QuickNavModal({ isOpen, onClose }: QuickNavModalProps) {
  ...
  const navItems = [
    ...
    { name: isAr ? 'المحاسبة' : 'Accounting', path: '/expenses?tab=accounting', icon: FileText, permission: 'view_finance', desc: isAr ? 'إغلاق الحسابات ومطابقات القيود' : 'Account balancing & matching settlements' },
    ...
    { name: isAr ? 'التقارير' : 'Reports', path: '/expenses?tab=reports', icon: FileText, permission: 'view_reports', desc: isAr ? 'تقارير مالية وتحليلية شاملة' : 'Detailed analytics & financial reports' },
