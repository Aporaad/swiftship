...
import { calculateShipmentCategoryFees, ItemCategory } from '../../services/itemCategoryService';
...
export default function ShipmentFormModal({
  ...
  const selectedContentCategory = itemCategories.find((category) => category.id === shipmentFormData.contentCategoryId);
  const categoryFees = calculateShipmentCategoryFees(selectedContentCategory, shipmentFormData.cartonCount);

  const updateContentCategory = (categoryId: string, cartonCount = shipmentFormData.cartonCount) => {
    const category = itemCategories.find((entry) => entry.id === categoryId);
    const fees = calculateShipmentCategoryFees(category, cartonCount);
    setShipmentFormData({
      ...shipmentFormData,
  ...
  const handleSaveShipmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const freshFees = calculateShipmentCategoryFees(selectedContentCategory, shipmentFormData.cartonCount);
      const payload = {
        id: shipId,
        ...
        cartonCount: freshFees.cartonCount,
        customsFee: freshFees.customsFee,
        taxFee: freshFees.taxFee,
        otherCategoryFee: freshFees.otherCategoryFee,
        categoryFeesTotal: freshFees.total,
        categoryFeeCurrency: freshFees.currency,
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 text-start">
          ...
          >
            ...
            <div className="p-6 overflow-y-auto space-y-4 flex-1 text-xs">
              ...
              <section className="space-y-3 rounded-2xl border border-[#d4af37]/25 bg-[#d4af37]/5 p-4" data-testid="shipment-content-category-fees">
                <div>
                  <h4 className="text-xs text-[#d4af37] font-black">{isAr ? 'فئة محتوى الشحنة ورسومها' : 'Shipment content category & fees'}</h4>
                  <p className="text-[10px] text-slate-500 mt-1">{isAr ? 'تُحسب الجمارك والضرائب لهذه الشحنة فقط حسب عدد الكراتين، ولا تُضاف إلى إجمالي الطلب.' : 'Customs and tax are calculated for this shipment only by carton count and are not added to the order total.'}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="block text-[10px] font-black text-slate-400 uppercase">{isAr ? 'فئة المحتوى' : 'Content category'}</label>
                    <select value={shipmentFormData.contentCategoryId || ''} onChange={(e) => updateContentCategory(e.target.value)} className="w-full bg-black/40 border border-slate-800 rounded-xl p-3 outline-none text-white font-bold cursor-pointer">
                      <option value="">{isAr ? '-- اختر فئة المحتوى --' : '-- Select content category --'}</option>
                      {itemCategories.map((category) => <option key={category.id} value={category.id}>{isAr ? category.nameAr : category.nameEn}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="block text-[10px] font-black text-slate-400 uppercase">{isAr ? 'عدد الكراتين' : 'Carton count'}</label>
                    <input type="number" min="0" step="0.01" value={shipmentFormData.cartonCount || 0} onChange={(e) => updateContentCategory(shipmentFormData.contentCategoryId || '', Number(e.target.value) || 0)} className="w-full bg-black/40 border border-slate-800 rounded-xl p-3 outline-none text-white font-bold font-mono" />
                  </div>
                </div>
                {selectedContentCategory && <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px] font-bold">
                  <FeeCard label={isAr ? 'جمارك' : 'Customs'} value={categoryFees.customsFee} currency={categoryFees.currency} color="text-amber-300" />
                  <FeeCard label={isAr ? 'ضريبة' : 'Tax'} value={categoryFees.taxFee} currency={categoryFees.currency} color="text-blue-300" />
                  <FeeCard label={isAr ? 'رسوم أخرى' : 'Other'} value={categoryFees.otherCategoryFee} currency={categoryFees.currency} color="text-purple-300" />
                  <FeeCard label={isAr ? 'الإجمالي' : 'Total'} value={categoryFees.total} currency={categoryFees.currency} color="text-emerald-300" />
                  {selectedContentCategory.requiresReview && <p className="col-span-full text-amber-400">{isAr ? 'تحتاج هذه الفئة مراجعة تنظيمية قبل اعتماد الرسوم.' : 'This category requires regulatory review before approving fees.'}</p>}
                </div>}
              </section>
...
function FeeCard({ label, value, currency, color }: { label: string; value: number; currency: string; color: string }) {
  return (
    <div className="rounded-xl bg-black/30 border border-slate-800 p-2">
      <span className="block text-slate-500">{label}</span>
      <span className={`font-mono ${color}`}>{Number(value || 0).toLocaleString()} {currency}</span>
    </div>
  );
}
