...
export default function Users() {
  ...
  const getEmployeeOrderPartyStats = (employeeId: string) => {
    const orders = allOrders.filter((order) =>
      (order.orderPartyType === 'employee' || (order.isStaffOrder && order.employeeId))
      && String(order.orderPartyId || order.customerId || order.employeeId) === String(employeeId),
    );
    return {
      total: orders.length,
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-gradient-to-b from-[#121215] to-[#08080a] border border-[#d4af37]/25 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden font-sans flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleUpdateUser} className="p-6 space-y-4 text-start overflow-y-auto flex-1">
              {getEmployeeOrderPartyStats(selectedUser.id).total > 0 && (
                <div className="rounded-2xl border border-violet-500/25 bg-violet-500/5 p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      <h4 className="text-xs font-black text-violet-200">{isAr ? 'طلبات مرتبطة بالموظف كطرف للطلب' : 'Orders linked to employee as order party'}</h4>
                      <p className="mt-0.5 text-[10px] font-bold text-slate-500">{isAr ? 'لا تدخل ضمن مهام الشحن أو مستحقات الراتب.' : 'Separate from logistics work and salary obligations.'}</p>
                    </div>
                    <span className="rounded-lg border border-violet-500/20 bg-violet-500/10 px-2 py-1 text-[10px] font-black text-violet-200">{getEmployeeOrderPartyStats(selectedUser.id).total}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div><span className="block text-[8px] font-black text-slate-500">{isAr ? 'القيمة' : 'Value'}</span><span className="font-mono text-[11px] font-black text-white">{getEmployeeOrderPartyStats(selectedUser.id).value.toLocaleString()}</span></div>
                    <div><span className="block text-[8px] font-black text-slate-500">{isAr ? 'المدفوع' : 'Paid'}</span><span className="font-mono text-[11px] font-black text-emerald-400">{getEmployeeOrderPartyStats(selectedUser.id).paid.toLocaleString()}</span></div>
                    <div><span className="block text-[8px] font-black text-slate-500">{isAr ? 'المتبقي' : 'Due'}</span><span className="font-mono text-[11px] font-black text-amber-300">{getEmployeeOrderPartyStats(selectedUser.id).outstanding.toLocaleString()}</span></div>
                  </div>
                </div>
              )}
              <div>
